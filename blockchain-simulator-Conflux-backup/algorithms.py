...
class GHOST(Algorithm):
    ...
    def fork_choice_rule(self):
        ...
        for vertex in max_subtree_vertices:
            ######## children[i] = (from vertex #, to vertex #) 是个edge
...
class Conflux(GHOST):
    ...
    def get_unlinked_blocks(self):
        ...
        blocks = []
        vertices = [self.root]   
        blocks.append(self.vertex_to_blocks[self.root])
        children = []
        children+=list(self.root.out_edges())
        for edge in children:
            target = edge.target()
    ...
    def topology_sorting(self, local_pivot_chain):
        ...
        while b_i < len(local_pivot_chain):
            ...
            epoch_i = self.DFS(local_pivot_chain[b_i], epoch_i=[], epochs=epochs)
    ...
    def DFS(self, block, epoch_i, epochs):
        ...
        if flag_notInEpochs:   
            ...
            if parent_id is not None:
                ...
                epoch_i = self.DFS(parent_block, epoch_i=epoch_i, epochs=epochs)
            if len(ref_ids) != 0:
                ref_ids = block.ref_ids
                for i, ref_id in enumerate(ref_ids):
                    ref_block = self.get_block_by_id(ref_id)
                    epoch_i = self.DFS(ref_block, epoch_i=epoch_i, epochs=epochs)
            return epoch_i
        else:
            return epoch_i
