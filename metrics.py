    ...
    with open('./logs/transactions.csv', newline='') as csvfile:
        ...
        for row in reader:
            ...
            if row['pool block reference timestamp']!='None':
                pool_block_ref_sum += float(row['pool block reference timestamp']) - float(row['generated timestamp'])
...
def dump_results():
    print('Results:')
    avg_pool_block_arr_latency, avg_pool_block_ref_latency, avg_main_chain_arrival_latency, avg_finalization_latency = compute_latency() 
